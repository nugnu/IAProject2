...
class SudokuGeneticProblem(GeneticProblem):
    ...
    def __get_selection_dict(self):
        return {
            ...
            1: self.__roulette_pick,
            "roulette": self.__roulette_pick
        }
    ...
    def __initEmptyArray(self):
        # returns the indices of the zero elements on the sudoku puzzle
        return np.sort(np.array(np.nonzero(self.initial == 0)[0]))
    ...
    def __init__(self, initial, f_threshold=None, initial_population_size=1000,
                 ...
                 selection_type="roulette", replacement_type="elitism", init_type="row_permutation", verbose=True):
        ...
        if (verbose == True):
            print("INITIAL PROBLEM AFTER NAKED_SINGLES PRE-PROCESSING:\n" +
                  self.to_string(self.initial) + "\n")
        # each position of this array keeps the position of an empty position on a sudoku puzzle
        self.emptyArray = self.__initEmptyArray()
    ...
    def goal_test(self, individual):
        ...
        # no threshold -> its expecting an actual solution (best possible fitness)
        else:
            if (self.max_ngen != None):
            ...
            for j in range(self.__SUDOKU_ARRAY_SIZE):
                if (j not in self.emptyArray):
                    ...
                    new_individual.append(
                        self.gene_pool[random.randrange(0, g)])
            self.population.append(new_individual)
    ...
    def __roulette_pick_util(self):
        total = sum(self.probabilities)
        r = random.uniform(0, total)
        upto = 0
        for c, w in zip(self.population, self.probabilities):
            if upto + w >= r:
                return c
            upto += w
        assert False, "Erro"
    ...
    def __flip_mutate(self, x):
        g = len(self.gene_pool)
        # select a random position of the empty array
        c = random.randrange(0, len(self.emptyArray))
    ...
    def __swap_mutate(self, x):
        ...
        # select a random position of the empty array
        c1 = random.randrange(0, empty_array_len)
        # select another random position of the empty array
        c2 = random.randrange(0, empty_array_len)
    ...
    def __horizontal_swap_mutate(self, x):
        ...
        # the position on the empty array that the row starts
        start_pos = np.searchsorted(self.emptyArray, 9*row)
        # the position on the empty array that the row ends
        end_pos = np.searchsorted(self.emptyArray, 9*row + 9)
    ...
    def __default_replace(self, prev_fitness_individual):
        ...
        newpopulation = []
        newpopulation = [self.mutate(self.crossover(*self.selection(2))) for i in range(len(self.population))]
    ...
    # similar to default but the population doesnt get bigger (same size as initial population)
    def __elitism_replace(self, prev_fitness_individual):
        # apply crossover to 2 random individuals (more chance to the fittest) and them apply the mutation
        # do it len(population) times
        newpopulation = []
        newpopulation = [self.mutate(self.crossover(*self.selection(2))) for i in range(len(self.population) - 1)]
        self.population = newpopulation
        # if needed, add the fittest of previous generation
        if (prev_fitness_individual != None):
            self.population.append(prev_fitness_individual)
